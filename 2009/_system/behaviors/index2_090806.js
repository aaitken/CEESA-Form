	...
	init:function(){
		...
		navButtonClick=function(instigator){
			fieldsets.each(function(i){
				if($(this).is(':visible')){
					$(this).hide();//hide current fieldset
					...
					instigator.id==='ama-next'?	
						...
						(function(){
							...
							}//function to add last row's attendee to attendee list on next step}
...
shared={
	assignIds:function(step){//kickoffEvent=whether init, add or remove is calling this;columnPrefixes=array of ID prefixes that correspond to the colomn fields within a single row
		var rowIterator=function(rows,columnPrefixes){
			...
			rows.each(function(){
				...
				$(this).find('td').each(function(){
					if($(this).find('input').is('input')){element=$(this).find('input')}
...
conferenceAttendees={
	...
	init:function(){
		...
		nameField.bind('change',function(){//this function applies updates made on step1 fields to step2 also
			var index,
				nextTabIndex,
				that;
			that=this;
